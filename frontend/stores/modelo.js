...
export const useModelosStore = defineStore("modelos", {
  ...
  actions: {
    ...
    async editModelo(modeloData) {
      try {
        const response = await axios.put(
          `http://127.0.0.1:8000/modelos/${modeloData.id}`,
          modeloData
        );
    ...
    async deleteModelo(modeloId) {
      try {
        const response = await axios.delete(
          `http://127.0.0.1:8000/modelos/${modeloId}`
        );
    ...
    async fetchPaginasDisponibles() {
      try {
        const response = await axios.get("http://127.0.0.1:8000/paginas");
        console.log(response.data);
        return response.data;
      } catch (error) {
        this.error = error.response?.data?.mensaje || error.message;
    ...
    async fetchGananciaInfo(nombreUsuario, nombrePeriodo) {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/ganancias/usuario/${nombreUsuario}/periodo/${nombrePeriodo}`
        );
    ...
    async crearDeduccion(nombreUsuario, DeducibleData) {
      try {
        const response = await axios.post(
          `http://127.0.0.1:8000/modelos/${nombreUsuario}/creardeducible`,
          DeducibleData
        );
        console.log(response.data);
        return response.data;
